...
public class RegionCondition extends ConditionDescriptor {
	...
	public boolean matches(DeathContext context) {
		...
		if (wgPlugin == null) {
			...
			if (region == null) {
				DeathControl.instance.log(Level.FINE, "Region condition: WorldGuard region " + regionName + " unexistent in world " + deathLoc.getWorld().getName() + "!");
				// not existent in this world
				return false;
			}
